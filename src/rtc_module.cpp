...
RtcDateTime getCurrentTime() { return Rtc.GetDateTime(); }

void setRTCTime(const RtcDateTime& dt) { Rtc.SetDateTime(dt); }
...
void syncRTCWithNTP() {
    ...
    RtcDateTime localTime =
        RtcDateTime(year(local), month(local), day(local), hour(local), minute(local), second(local));
